...
def create_grid_image(res=50, cell_size=12, header_size=12):
    ...
    # Draw x headers left-to-right along the top edge.
    for j in range(cols):
        text = str(j +1)
    ...
    # Draw y headers top-to-bottom along the left edge.
    for i in range(rows):
        text = str(i + 1)
        text_bbox = draw.textbbox((0, 0), text, font=font)
        text_width = text_bbox[2] - text_bbox[0]
        ...
        text_y = (i + 1) * cell_size + (cell_size - text_height) / 2 - 0.2*text_height
        draw.text((text_x, text_y), text, fill="black", font=font)
    
    # Separate the top/left headers from the drawing cells.
    draw.line([(cell_size, 0), (cell_size, img_height)], fill="black")
    draw.line([(0, cell_size), (img_width, cell_size)], fill="black")
    
    positions={}
    for i in range(rows):
        for j in range(cols):
            if j == 0:
                draw.rectangle([(0, (i + 1) * cell_size), (cell_size, (i + 2) * cell_size)], outline="black")
            if i == 0:
                draw.rectangle([((j + 1) * cell_size, 0), ((j + 2) * cell_size, cell_size)], outline="black")

            text = f"x{j + 1}y{i + 1}"
            center_y = int((i + 1) * cell_size + cell_size / 2)
            center_x = int((j + 1) * cell_size + cell_size / 2)
...
def cells_to_pixels(res=50, cell_size=12, header_size=12):
    ...
    positions={}
    # Standard screen coordinates: y1 is the top row and y increases down.
    for i in range(rows):
        for j in range(cols):
            text = f"x{j + 1}y{i + 1}"
            center_y = int((i + 1) * cell_size + cell_size / 2)
...
def parse_xml_string(llm_output, res):
    ...
    strokes_node = root.find('strokes')
    if strokes_node is None:
        raise ValueError("SketchAgent XML is missing <strokes>")
    for stroke in strokes_node:
        # Extract points and clean them up
        points_node = stroke.find('points')
        t_values_node = stroke.find('t_values')
        if points_node is None or not (points_node.text or '').strip():
            raise ValueError(f"stroke {stroke.tag} is missing non-empty <points>")
        if t_values_node is None or not (t_values_node.text or '').strip():
            raise ValueError(f"stroke {stroke.tag} is missing non-empty <t_values>")
        points_text = points_node.text
        # The original benchmark grammar uses quoted integer grid cells, but
        # some providers omit quotes or emit half-grid coordinates. Normalize
        # only this local syntax so semantic drawing content is preserved.
        def normalize_cell(match):
            x = max(1, min(res, round(float(match.group(1)))))
            y = max(1, min(res, round(float(match.group(2)))))
            return f"'x{x}y{y}'"
        points_text = re.sub(
        ...
        if points_text.startswith("[") and points_text.endswith("]"):
            points_text = points_text[1:-1].strip()

        # Extract t_values and convert them to float
        t_values_text = (t_values_node.text or "").strip()
        if t_values_text.startswith("[") and t_values_text.endswith("]"):
            t_values_text = t_values_text[1:-1].strip()
